...
public class Jackson2Converter {
    ...
    public <T> T toBean(String json, Class<T> clazz1, Class<?> clazz2) throws IOException {
        JavaType javaType = getMapper().getTypeFactory().constructParametrizedType(clazz1, clazz1, clazz2);
        return toBean(json, javaType);
    }
    public <K,V> Map<K,V> toMap(String json ,Class<K> keyClass,Class<V> valClass)throws IOException{
        JavaType javaType = getMapper().getTypeFactory().constructMapType(Map.class,keyClass,valClass);
        return toBean(json, javaType);
    }
    /**
    ...
    public <T> T toBean(String json, Class<T> clazz, Type type) throws IOException {
        ...
        if (type == null)
            ...
            ClazzUtils.getGenericClass(type, genericClass);
            JavaType javaType = null;
            if (genericClass.getRawClass() != null) {
                javaType = ObjectMapperUitls.getJavaType(this.mapper,genericClass);
            }
            if (javaType != null) {
    ...
    public String getNoteValue(JsonNode node, String path) {
        if (!StringUtils.isEmpty(path)) {
            ...
            for (int i = 0; i < nodeNames.length; i++) {
                ...
                if (i == nodeNames.length - 1) return node.asText();
            }
        }
        return null;
    }
    ...
    /**
     * 排除某些字段不输出.如:<br/>
     * TestBean里有id,name两个属性(实际上是以get方法为准)只想输出name属性<br/>
     * <code>
     *
     * @param filterName
     * @param propertys
     * @JsonFilter("testFilter")</br> class TestBean{</br>
     * }</br>
     * JsonConverter.addFilter("testFilter",new String[]{"id"});
     * </code>
     */
    public void addFilter(String filterName, String[] propertys) {
        ...
        simpleFilterProvider.addFilter(filterName, SimpleBeanPropertyFilter.serializeAllExcept(propertys));
